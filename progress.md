...
- [x] H阶段自动上报评测对齐：0x0302开始(首帧立即回+按间隔持续发UTC+CH0+CH1)、0x0303停止(回OK)、自动上报期间屏蔽其他命令(H-02)

## 当前调试问题
- 【搁置】BL OTA接收：0x0502帧在倒计时期间无法被BL正确接收并解析
...
## 待开发任务
- 【搁置·N-01/N-02/N-03】BL OTA接收流程无法通过评测

  **已完成部分：**
  - USART1 DMA接收初始化、空闲中断、`USART1_IRQHandler`均正常工作（irq计数验证）
  - `bl_parse_cmd` CRC16/帧格式解析逻辑验证正确（Python离线计算与实际数据吻合）
  - `bl_puts`/`bl_send_frame` RS485方向控制、禁用空闲中断防回声均已实现
  - 0x0501→APP回OK→RTC备份寄存器标记→复位→BL倒计时打印全部正常

  **困难点：**
  1. RS485半双工：`bl_puts`发送倒计时字符串期间PE8=1，USART1物理上无法接收，发完切回接收后DMA需重置，但重置操作本身会清掉这段时间内到达的数据
  2. `bl_send_frame`发心跳帧后上位机立即发0x0502，心跳帧发送期间或发完后的DMA重置会把0x0502清掉
  3. 调试确认：irq中断触发正常、buf数据内容正确（`41 35 42 36...0D 0A 0D 0A`），但`usart1_rx_flag`在轮询窗口内未能稳定保留有效帧数据供解析
  4. 根本矛盾：RS485发送和接收无法同时进行，发送倒计时/心跳帧与接收0x0502之间存在时序竞争，尚未找到可靠的同步方案
