...
#### D/E 阶段剩余（当前进行中）
- [ ] **编译验证**：config结构扩展+新命令分支（0x0202/0x0221/0x0241/0x0242/0x0400~0x0412）全部编译通过
- [ ] **F/G 持久化验证**：重启后CH0/CH1变比、阈值、设备ID、波特率均从Flash正确恢复

#### J 阶段：睡眠唤醒（5分，未实现）
- [ ] 0x03AA：回OK后进MCU深度睡眠（Stop模式或DeepSleep）
- [ ] RTC闹钟10s后自动唤醒
- [ ] 唤醒后发字符串 `instrument wakeup`（非帧封装，ASCII直接回复）

#### I 阶段：告警完整实现（7分，部分实现）
- [ ] 0x0601：设置是否主动上报告警（01=主动，02=仅存储）
- [ ] 0x0602：查询告警记录（最近10条，倒序，字符串格式：`时间|通道|阈值|实际值`，无记录回`empty`）
- [ ] 0x0603：清除告警记录（回OK）
- [ ] 告警记录持久化：写入外部Flash（GD25QXX）或内部DATA区
- [ ] 告警触发：采样时超阈值→存储+（如主动上报则发字符串）
- [ ] 告警格式：`2026-01-01 12:00:00 | CH0 | 10.50 | 11.12\r\n`
...
#### C 阶段：时间设置回读验证（2分）
- [ ] 串口验证：发0x0105设UTC时间戳，回读0x0106确认一致
