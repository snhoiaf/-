...
- [~] **T6 N 阶段 BL OTA**（代码已重构，待烧录验证）：对齐满分BL字节级接收，解决RS485半双工时序竞争
  ...
  - ⚠️【评测N仍扣分-真凶3】2026-06-08评测36.5分(A~M全绿)，N阶段失败：**BL波特率写死19200，但评测M阶段后上位机/参数已切115200**。N-01关键字(BL@19200)被上位机(115200)收成乱码；0x0502(115200)被BL(19200)收成乱码不响应→BL超时跳回APP→固件被APP接收(日志见`[U1]256by`)。
    - 待修：BL应从参数页读取持久化波特率(comm_baud_code)跟随APP，对齐满分。我方BL `bsp_usart1_init`写死`usart_baudrate_set(USART1,19200)`需改为读参数页
  - 改动7【修真凶3】：BL新增`bl_read_app_config`读APP配置(0x0807D000的config_data_t,magic"CONF"+CRC32校验)，`bootloader_wait_cmd`进入时据此同步USART1波特率+应答ID跟随APP。解决评测M阶段切115200后BL仍19200的失配。待烧录验证
  - ✅【真凶3已验证】2026-06-08模拟评测脚本(ota_test_115200.py)：先切APP到115200(模拟M阶段)，上位机保持115200跑OTA，BL成功跟随115200，0x0501→BL关键字→0x0502→固件校验→0x0503 全PASS！N阶段波特率失配根除
**修复记录**：
